...
async function loadShopIcons(category) {
  const [categoryIcon, roseIcon, checkIcon] = await Promise.all([
    getIcon(CATEGORY_ICONS[category] ?? 'gift'),
    getIcon('rose'),
    getIcon('check'),
  ]);
  return { categoryIcon, roseIcon, checkIcon };
}

/** Dessine une frame complete de la carte boutique sur ctx, pour la phase
 * d'animation t (0..1). Partage par la version animee (GIF) et statique (PNG). */
function drawShopFrame(ctx, { category, items, roses, page, totalPages }, icons, t) {
  const { categoryIcon, roseIcon, checkIcon } = icons;
 ...
 */
async function renderShopGif(opts) {
  const icons = await loadShopIcons(opts.category);
  const frames = [];

  for (let f = 0; f < FRAMES; f++) {
    const canvas = createCanvas(W, H);
    const ctx = canvas.getContext('2d');
    drawShopFrame(ctx, opts, icons, f / FRAMES);
    frames.push(ctx);
  }
...
/**
 * Version statique (une seule image, pas d'animation) : ~10x plus rapide a
 * generer que renderShopGif, utilisee pour la pagination et apres un achat
 * ou la reactivite compte plus que l'animation.
 * @param {Parameters<typeof renderShopGif>[0]} opts
 * @returns {Promise<Buffer>} PNG
 */
async function renderShopPng(opts) {
  const icons = await loadShopIcons(opts.category);
  const canvas = createCanvas(W, H);
  const ctx = canvas.getContext('2d');
  drawShopFrame(ctx, opts, icons, 0.25);
  return canvas.encode('png');
}

module.exports = { renderShopGif, renderShopPng };
